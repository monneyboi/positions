...
2. Running `positions` opens a TUI that derives each eligible P17/P1001
   proposal from the local model on the fly, ordered by P39 usage. While you
   decide, the next proposal is already selected in the background. Nothing
   is stored except your decisions.
3. Accept re-fetches the position and its body, verifies the proposal against
   live Wikidata, and submits both claims in one revision-guarded edit. Every
   discard or approval is persisted in the decision table as a tombstone, so
   decided positions are never proposed again.
...
uv run positions                      # review TUI: accept, discard, or quit
